...
import { createPositionChart, createBallResponseChart, createFlowChart, createSeriesChart, createEsdCycleChart, CHART_COLORS } from '../dashboard/anomalyCharts.js';
import { chartKind, seriesChartFor, esdCycleSummary } from '../dashboard/anomalyAnalytics.js';
...
export function createMaintenanceUI({ onOpenComponent }) {
  ...
  function analyticsHtml(componentId, detectedAt) {
    ...
    return `<div class="mt-section"><h4>Current Values</h4><div class="mt-kv" data-live-values>${liveKv(componentId)}</div></div>
      <div class="mt-section"><div class="mt-range"><h4 style="margin:0">${chartKind(componentId) === 'position' ? 'Command vs Actual Position' : chartKind(componentId) === 'ballResponse' ? 'Ball Valve Open/Close Response' : chartKind(componentId) === 'esdResponse' ? 'ESD Trip Response' : chartKind(componentId) === 'esdCycle' ? `ESD Valve Command vs Actual State — ${esdCycleSummary()?.current || 0} Cycles` : chartKind(componentId) === 'series' ? (seriesChartFor(componentId)?.title || 'Trend') : 'Steam Flow vs Expected'}</h4>
  ...
  function mountChart(componentId, detectedAt) {
    ...
    view.chart = kind === 'position' ? createPositionChart(host) : kind === 'ballResponse' || kind === 'esdResponse' ? createBallResponseChart(host) : kind === 'esdCycle' ? createEsdCycleChart(host) : kind === 'series' ? createSeriesChart(host) : createFlowChart(host);
    view.chartKind = kind;
  ...
  function updateAnalytics(componentId, detectedAt) {
    ...
    if (view.chart && bodyEl.querySelector('[data-chart] svg')) {
      ...
      else if (view.chartKind === 'esdCycle') { const ct = S.esdValve.cycleTest; if (ct) { const now = Date.now(), wall = (st) => now - (S.time - st) * 1000; view.chart.update(esdSamplesWallClock(), { startAt: wall(ct.startSt), endAt: wall(ct.active ? S.time : ct.endSt), period: ct.period, total: ct.total, cycles: ct.cycles }, { acceptable: S.esdValve.sim.acceptableDelay }); } }
